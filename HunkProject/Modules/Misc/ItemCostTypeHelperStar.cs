...
{
    internal class ItemCostTypeHelperStar
    {
        public static bool IsAffordable(CostTypeDef costTypeDef, CostTypeDef.IsAffordableContext context)
        {
            ...
            int itemCount = inv.GetItemCount(Modules.Survivors.Hunk.goldKeycard);
            ...
            else
            {
                ...
                if (hunk)
                {
                    hunk.notificationHandler.SoftInit("You need a U.C. Keycard (Star) to open this", UnityEngine.Color.red, 2f);
                }
                return false;
            }
        }

...
{
    public class ElderGrenade : BaseHunkSkillState
    {
        ...
        public override void OnEnter()
        {
            ...
            base.PlayAnimation("FullBody, Override", "CounterElderGrenade", "Dodge.playbackRate", this.duration);
            ...
            this.desiredPos = this.targetPos + new Vector3(0f, 1f, 0f) + (this.lookVector * 1.3f);
        ...
        public override void FixedUpdate()
        {
            ...
            if (!this.hasDroppedPin && base.fixedAge >= 0.3f * this.duration)
            {
                this.hasDroppedPin = true;

                this.desiredPos = this.targetPos + new Vector3(0f, 3.8f, 0f) + (this.lookVector * 3f);
            ...
            if (!this.hasDroppedGrenade && base.fixedAge >= 0.6f * this.duration)
            {
                ...
                this.desiredPos = this.targetPos + new Vector3(0f, 4f, 0f) + (this.lookVector * 4f);
                //this.lerpSpeed *= 2f;

                if (this.grenadeInstance) Destroy(this.grenadeInstance);
                if (this.target) this.target.gameObject.AddComponent<Modules.Components.MouthGrenade>().attackerBody = this.characterBody;

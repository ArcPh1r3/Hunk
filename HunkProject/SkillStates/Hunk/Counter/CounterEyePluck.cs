...
namespace HunkMod.SkillStates.Hunk.Counter
{
    public class CounterEyePluck : BaseHunkSkillState
    {
        ...
        public override void OnEnter()
        {
            ...
            base.PlayAnimation("FullBody, Override", "CounterEyePluck", "Dodge.playbackRate", this.duration);
            ...
            this.desiredPos = this.targetPos + (this.lookVector * 2f);
        ...
        {
            if (this.eyeInstance) Destroy(this.eyeInstance);
        ...
        public override void FixedUpdate()
        {
            ...
            if (this.target && NetworkServer.active)
            {
                ...
                if (this.hasSnapped) this.target.body.characterMotor.Motor.SetPosition(this.targetPos);
                else this.target.body.characterMotor.Motor.SetPosition(this.targetPos + new Vector3(0f, -1f, 0f));
            ...
            if (!this.hasSnapped && base.fixedAge >= 0.42f * this.duration)
            {
                ...
                if (this.target)
                {
                    if (NetworkServer.active)
                    {
                        this.target.body.characterMotor.Motor.SetPosition(this.targetPos + new Vector3(0f, -1f, 0f));
                    }
        ...
        {
            if (base.fixedAge >= 0.43f * this.duration) return InterruptPriority.Skill;
